...
const getMyProfile = async (req, res) => {
  ...
  if (!profile) {
    return res.send({ success: false, error: 'This user does not exist.' });
  }
...
const getProfile = async (req, res) => {
  ...
  if (login !== loggedUser) {
    ...
    if (!profile) {
      return res.send({ success: false, error: 'This user does not exist or is blocked or closed his account.' });
    }
...
const getVisitsOrLikesOrMatches = search => async (req, res) => {
  ...
  const profile = await Getter.getUser(login);
